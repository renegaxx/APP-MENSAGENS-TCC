...
import { View, Text, TextInput, TouchableOpacity, FlatList, StyleSheet, Image, Animated, Alert } from 'react-native';
import { collection, addDoc, onSnapshot, query, where, orderBy, doc, getDoc, updateDoc, arrayUnion, deleteDoc } from 'firebase/firestore';
...
const MessagesScreen = ({ route, navigation }) => {
  ...
  const handleSendMessage = async () => {
    if (message.trim()) {
      await addDoc(collection(db, 'messages'), {
        ...
        timestamp: new Date(),
        clearedBy: [], // Initialize as an empty array
      });
      setMessage('');
    }
  };
  ...
  // Renderizar sugestão de mensagem se não houver mensagens
      ...
      <View style={styles.suggestionContainer}>
        ...
        <View style={styles.suggestionButtonsContainer}>
          {suggestions.map((text, index) => (
            <TouchableOpacity key={index} style={styles.suggestionButton} onPress={() => setMessage(text)}>
  ...
  return (
    <View style={styles.container}>
      ...
      {messages.length === 0 ? (
        renderSuggestions() // Exibe sugestões se não houver mensagens
      ) : (
        <FlatList
          data={messages}
      ...
      <View style={styles.componentes}>
        <TouchableOpacity onPress={handleClearHistory} style={styles.batePapo}>
          <Image source={require('./assets/icons/lixeiraImg.png')} style={styles.trashIcon} />
...
const styles = StyleSheet.create({
  ...
  messageText: { color: '#fff', fontFamily: 'Inter-Regular', fontSize: 16 },
  inputContainer: {
    flexDirection: 'row',
    padding: 11,
    alignItems: 'center',
  ...
  componentes: { marginRight: 'auto', flexDirection: 'row', paddingHorizontal: 14, height: 50 },
  batePapo: { flexDirection: 'row', alignItems: 'center' },
  trashIcon: { width: 22, height: 22, resizeMode: 'cover' },
  clearHistoryText: { color: '#A1A0A0', marginLeft: 5, fontFamily: 'Montserrat-Regular', width: 125 },
  suggestionContainer: {
    alignItems: 'center',
    padding: 20,
  },
